...
function orderFunc(){
    ...
    //특정 상황에서 아래 orderSelect가 출력되고 싶다면? 상황을 조건문으로 만들어야한다
    //orderSelect 출력 조건 -> color의 value 값이 존재하는 option 선택 후 size의 value값이 존재하는 option을 추가로 선택했을 경우
    ...
    if(colorSelect.value && sizeSelect.value){   //colorSelect의 값이 참이면 
        orderSelect.style.display = 'block';  //이걸 실행해라 (주문영역 보이기)
        //order_select의 color 안에 선택한 colorSelect 값 대입하기
        orderColor.textContent = colorSelect.value;
        //order_select의 size 안에 선택한 sizeSelect 값 대입하기
...
//감소 - 버튼 클릭 시 수량이 1씩 감소되어 ordernum 출력
minusBtn.addEventListener('click',()=>{
    num--;
    //감소 수량이 1보다 작은 경우 경고메세지를 출력
    if(num < 1){
        alert('1~9999개까지 선택가능합니다.')
    }
    if(num >= 1){
...
//장바구니 클릭 시 '장바구니에 담겼습니다' 메세지 alert()
const cart = document.querySelector('#cart_btn');
//바로구매 클릭 시 '로그인 해주세요' 메세지
const buyNow = document.querySelector('#buy_btn');
console.log(cart,buyNow);
//cart.addEventListener('click',()=>{
//    alert('장바구니에 담겼습니다');
//})
cart.addEventListener('click',cartmsg);
function cartmsg(){
    alert('장바구니에 담겼습니다');
}
//buyNow.addEventListener('click',()=>{ 화살표 함수 이건 간략히 쓰는게 목적
//    alert('로그인 해주세요');
//})
buyNow.addEventListener('click',buyNowMsg);   //이건 반복할때 주로 사용 (함수호출)
function buyNowMsg(){
    alert('로그인 해주세요');
}   
/* buyNow.addEventListener('click',function(){  익명함수
    alert('로그인 해주세요');
}); */