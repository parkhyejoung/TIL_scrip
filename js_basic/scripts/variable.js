...
var num1 = 10;
var num1 = 20;
//var는 중복선언이 가능하다. 하지만 실무에선 이미 생성한 변수명을 또 중복선언하는 문제를 내면 안되기때문에 var 이를 오류로 선언하지 못해서 가능한 사용을 하지 않거나 주의해서 사용해야 한다.
let num2 = 30;
/* let num2 = 40; */
//let 변수생성키워드는 이미 생성한 변수를 중복선언 하지못하도록 체크하기 때문에 개발자의 실수를 방지할 수 있다.
const num3 = 50;
//const num3; 중복선언불가
/* num3 = 60; 값 변경 불가(처음 선언된 값을 끝까지 유지해야한다!!)*/ 
//const num4; /* 상수는 생성과 대입이 반드시 동시에 진행해야한다! */
num4 = 60; //생성과 대입이 동시에 된 올바른 경우 ((TIP. 오류체크를 자주하기))
//==================================객체 연습
...
console.log(user_info.age , user_info.name);
//3.쇼핑몰 상품 저장(상품명,가격,재고수량,색상)
let product = {
    name:'Cinnamon Rollcake',
    price:9500,
    inventory:3,
    color:'Cinnamon',
}
...
//ooo의 재고수량은 000이다.
console.log(`${product.name}의 재고수량은 ${product.inventory}이다`);
...
console.log(`오늘 ${movie_theater.seat_name}좌석 ${movie_theater.seat_location}번 ${movie_theater.seat_price}에 예약했다.`);
//오늘 00좌석 00번 00원에 예약했다.
//=================================배열 공부
//배열 작성법1. (값은 개수 제한 없음, 최소 2개이상)
//let 배열변수명 = [배열값, 배열값, 배열값];  <<가변성 0
//const 배열변수명 = [배열값, 배열값, 배열값]; << 가변성 x
