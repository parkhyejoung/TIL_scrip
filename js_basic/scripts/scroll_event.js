...
topBtn.addEventListener('click',(e)=>{
    e.preventDefault(); // 태그의 기본 이벤트 막기
    window.scrollTo(0,0);
})

//검색 폼 복제 변수
const searchClone = document.querySelector('header form').cloneNode(true); //선택 복제 한번에
let isSearchClone = false; // 안보이면 false 보이면 true

window.addEventListener('scroll',()=>{
    console.log(window.scrollY)
    //1.스크롤 이벤트에 따라 헤더 디자인변경
    ...
    if(window.scrollY > 100){
        ...
        if(isSearchClone){//(위)else 100보다 작으면 바로 실행하지않고 이중검사 후 실행
            headerBtm.classList.remove('active');
            headerBtm.removeChild(searchClone);
            isSearchClone = false;
        }
    }
    //2.스크롤 이벤트에 따라 맨 위로 변경
    //맨 위로 숨기기
    //스크롤 위치가 500보다 크면 보이기(반대면 숨기기)
