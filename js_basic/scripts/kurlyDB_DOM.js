...
let isLoading = false;//로딩완료 ture , 로딩미완료 false

console.log(container);

loadMoreContent() //처음보이는 제품나열을 위한 함수호출
function loadMoreContent(){
    ...
    //위 for문 한번 출력 후 스크롤이벤트로 인해 재출력 시 endIndex값이 다음출력되는 DB 개수 만큼 증가되게수정
    endIndex += itemLoad;
    isLoading = false;
}
...
/*     //1.요소가 뷰포트와의 거리가 어떻게 되는지 확인 변수
    ...
    //현재 뷰포트 하단 window.scrollY + window.innerHeight
    ...
    if(currentScroll >= totalHeight * scrollValue && !isLoading){
        ...
        if(endIndex >= kurlyDB.length){ return } //return = 함수종료
