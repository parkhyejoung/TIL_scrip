// 25/05/15 배열공부
/* const yoil = ['월','화','수']; */
const yoil = new Array('월','화','수','목','금','토','일'); /* 자바스크립트 내장스크립트 불러오는 거라서 () 괄호쓴다  */
/* console.log(yoil[0]);
console.log(yoil[1]);
console.log(yoil[2]); */
//====================색상배열
...
//(위)요일변수와 새로운 날씨 카드 배열 활용 문자 출력하기
...
const movieInfo_v2 = {
    ...
    genre:'어드벤처',
    rating:124,
    actor:['제이슨 모모아', '잭 블랙', '다니엘 브룩스', '엠마 마이어스', '세바스찬 한센'],
    releaseDate:'2025.04.26',
    age:12,
    story:"왕년의 게임 챔피언이었지만 지금은 폐업 직전의 게임샵 주인이 된 '개릿'과",
}
//동일한 분류의 객체가 많을 경우 묶는 방법
const movieAll = [ //데이터 베이스 DB
    {
        name:'미션임파서블',
        director:'크리스토퍼 맥쿼리',
        genre:'액션',
...
console.log(movieAll[0].releaseDate,movieAll[1].director); // . 은 객체 []은 배열 호출
//감독 : 크리스토퍼 맥쿼리
//장르 : 액션/169분
//등급 : 15세 이상 관람가 
console.log(`감독 : ${movieAll[0].director}`); /* 뭉치 > 뭉치 번호  > 가져올 값  */
...
//hello figma
console.log(greeting+lang[2]); /* 변수 > 뭉치 > 번호 */
...
//===================================빼기,곱하기,나누기 등 연산은 숫자가 문자로 따욤표가 묶여 있을 시 자동으로 숫자로 형변환시킨다.
const data1 = 1;
const data7 = "7";
console.log(data1+data7);//7
console.log(data7-data7);//0
console.log(data7*data7);//49
