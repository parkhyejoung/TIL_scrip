...
//2.(위) 오류없는거 확인 후 복제변수 생성
const productLiClone = productLi.cloneNode(true);
console.log(productLi,productLiClone);
...
const slideContainer = document.querySelectorAll('.slide_containar');
let num = 0;
...
//(위) 특징:타이머를 담은 변수명으로 타이머 정지,다시재생등의 제어를 할수있다.
...
console.log('C');
//(위) 동기(a,c)와 비동기(b) 순차적으로 실행되지않는다.
//동기작업(a,c)는 콜스택에 바로 쌓이며 즉시 실행되고
//비동기작업(b)는 콜백 큐에 저장되고 준비가 되면 순차적순서 상관없이 독립적으로 실행된다.
