...
//aniImg.styles.transform = 'translate(10px ,10px)';
...
inputMove.addEventListener('keydown',function(e){
    if(e.key === 'Enter'){
        ...
        }else if(inputMove.value === '오른쪽'){ //조건1이 거짓일때 검사하는 조건2
            currentX +=20;
            inputMovefunc()
        }else if(inputMove.value === '위'){ //조건1,2이 거짓일때 검사하는 조건3
            currentY -=20;
            inputMovefunc()
        }else{ //조건 1,2,3모두 거짓일때 실행결과
            currentY +=20;
            inputMovefunc()
        }
        inputMove.value = '';
